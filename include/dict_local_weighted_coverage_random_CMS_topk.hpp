...
public:
    static void create(collection& col, bool rebuild, size_t size_in_bytes)
    {
        ...
        if (!utils::file_exists(fname) || rebuild) { // construct
            ...
            LOG(INFO) << "\t"
                      << "Sketch size = " << cfe.sketch.size_in_bytes() / (1024 * 1024) << " MiB";
            ...
            if (!utils::file_exists(sketch_name) || rebuild) {
                ...
                auto stop = hrclock::now();
                LOG(INFO) << "\t"
                          << "Estimation time = " << duration_cast<milliseconds>(stop - start).count() / 1000.0f << " sec";
                LOG(INFO) << "\t"
                          << "Store sketch to file " << sketch_name;
                sdsl::store_to_file(cfe, sketch_name);
            }
            else {
                LOG(INFO) << "\t"
                          << "Load sketch from file " << sketch_name;
            ...
            LOG(INFO) << "\t"
                      << "Sketch params = {d=" << cfe.sketch.d << ",w=" << cfe.sketch.w << "}";
            LOG(INFO) << "\t"
                      << "Sketch estimation error = " << cfe.sketch.estimation_error();
            LOG(INFO) << "\t"
                      << "Sketch estimation confidence = " << cfe.sketch.estimation_probability();
            LOG(INFO) << "\t"
                      << "Sketch noise estimate = " << cfe_noise;
            LOG(INFO) << "\t"
                      << "Number of things hashed = " << cfe.sketch.total_count();
