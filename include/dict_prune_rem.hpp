...
struct dict_prune_rem {
    ...
    static void prune(collection& col, bool rebuild, uint64_t target_dict_size_bytes, uint64_t num_threads)
    {
        ...
        for (size_t i = 0; i < fstats.dict_usage.size(); i += t_block_size_bytes) {
            ...
            if (block_size > t_block_size_bytes)
                block_size = t_block_size_bytes;
        ...
        LOG(INFO) << "\t"
                  << "Find the smallest blocks we want to remove.";
        ...
        LOG(INFO) << "\t"
                  << "Removing " << num_blocks_to_remove << " (" << bytes_to_remove << " bytes)";
        ...
        LOG(INFO) << "\t"
                  << "Smallest block weight removed: " << block_weights[0].weight;
        LOG(INFO) << "\t"
                  << "Largest block weight removed: " << block_weights.back().weight;
