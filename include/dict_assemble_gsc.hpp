...
public:
    static void create(collection& col, bool rebuild, size_t size_in_bytes)
    {
        ...
        if (!utils::file_exists(fname) || rebuild) { // construct
            ...
            LOG(INFO) << "\t"
                      << "Assembling and initial dic construction...";
            auto start = hrclock::now();
            ...
            for (size_t i = 0; i < text.size() - t_estimator_block_size + 1; i++) {
                ...
                if (i < beginPos + t_estimator_block_size - 1)
                    continue;
                else {
                    if (block_counts.find(hash) != block_counts.end()) //assemble
                        ...
                        if (found == true && i - beginPos <= 1024) { //write while removing duplicates
                            ...
                            if (uniqueSegs.find(seg_hash) == uniqueSegs.end()) {
                                ...
                                numSeg++;
                                LOG(INFO) << "\t"
                                          << "Unique Segments: " << numSeg << "  Length: " << i - beginPos;
                            }
                            LOG(INFO) << "\t"
                                      << "Completed: " << (double)i * 100 / text.size() << "\%";
                            beginPos = i + 1;
                            found = false;
                        }
            ...
            uniqueSegs.clear();
            LOG(INFO) << "\t"
                      << "Final number of frequent segments = " << numSeg;
            LOG(INFO) << "\t"
                      << "Final dictionary size = " << dict.size() / (1024 * 1024) << " MiB";
            dict.push_back(0); // zero terminate for SA construction
            auto stop = hrclock::now();
            LOG(INFO) << "\t"
                      << "Assemble and construct dic runtime = " << duration_cast<milliseconds>(stop - start).count() / 1000.0f << " sec";
        }
        else {
            LOG(INFO) << "\t"
